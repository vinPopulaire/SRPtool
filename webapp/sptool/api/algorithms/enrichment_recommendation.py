...
def enrichments_recommendation_for_project(user_vector, project_id):
    ...
    # what enrichments are present in the video (only distinct this time
    video_enrichments = VideoEnrichments.objects.filter(project_id=project_id).distinct('enrichment_id')
